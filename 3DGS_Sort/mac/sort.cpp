...
void sort(int splatCount , int* splatID , float* camera , float* center)
{
    ...
    for (int i = 0; i < splatCount; i++)
    {
        ...
        y = center[i * 3 + 1];
        z = center[i * 3 + 2];
        bound_min_x = std::min(bound_min_x, x);
        bound_min_y = std::min(bound_min_y, y);
        bound_min_z = std::min(bound_min_z, z);
        bound_max_x = std::max(bound_max_x, x);
        bound_max_y = std::max(bound_max_y, y);
        bound_max_z = std::max(bound_max_z, z);
    }
    ...
    while (1)
    {
        ...
        */
        ...
        if (result[0] == "0")
        {
            ...
            //splatID
            int splatIdShm = shm_open(result[2].c_str() , flag );
            ...
            void* splatIDMapFileBuf = mmap(NULL, shm_size, 
            ...
            //camera
            int cameraShm = shm_open(result[3].c_str() , flag );
            ...
            void* cameraMapFileBuf = mmap(NULL, shm_size , 
                                          PROT_READ | PROT_WRITE , 
                                          MAP_SHARED,  
                                          cameraShm,
                                          0);
            ...
            //center
            int centerShm = shm_open(result[4].c_str() , flag );
            ...
            void* centerMapFileBuf = mmap(NULL, shm_size , 
                                          PROT_READ | PROT_WRITE , 
                                          MAP_SHARED,  
                                          centerShm,
                                          0);

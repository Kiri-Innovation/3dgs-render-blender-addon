"""Shared transform-bake + SH-rotation helpers for 3DGS/4DGS export.

Three byte-identical copies of these helpers used to live inside
``apply_3dgs_tranforms.py`` and the two branches of
``export_mesh_as_3dgs4dgs.py``. This module consolidates them so future
work (GPU SH rotation, custom PLY writer, batched IO) edits one site.

The public entrypoint is :func:`apply_transforms_to_object`, which mirrors
the inline ``main()`` that used to run in each operator's ``execute``.
"""
...
def apply_transforms_to_object(target_obj_name):
    """Public entrypoint mirroring the inline ``main()`` from the original operators."""
